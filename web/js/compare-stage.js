 ...
 *
 * Three things follow from putting six robots where the rest of the site puts
 * one:
 *
...
export class CompareStage {
      ...
      for (const entry of missing) {
        ...
        try {
          ...
          if (!this.members.has(entry.key)) {
            ...
            this.members.set(entry.key, { entry, robot, error: null });
          }
        } catch (err) {
          if (this.members.has(entry.key)) {
  ...
  /**
   * Stand them in a row, in the order the table reads them, at true scale and
   * on one floor.
   *
   * Each description is measured where it is drawn — no URDF declares how wide
   * a machine is any more than it declares how tall — so the row is laid out
   * from the geometry that actually arrived, and a robot whose meshes half
   * failed takes the width it has rather than the width it should have had.
   * The whole row is then centred on the origin, which is where the camera
   * frames and where the grid is drawn.
   *
   * The world group is Z-up (that is what a URDF means) while the scene is
   * Y-up, so a robot's own +X is the row, +Z is the height, and +Y is depth.
   */
  layout() {
    if (!this.viewer) return;
    ...
    // Measured with the turntable unwound, as everything that measures this
    // stage is: an axis-aligned box around a row stopped mid-spin is a box
    // around its diagonal, and the row is laid out along an axis the spin has
    // turned away from. `frameCamera` leaves it at zero, which is where it is
    // put back.
    for (const member of standing) member.robot.position.set(0, 0, 0);
    this.viewer.world.rotation.z = 0;
    this.viewer.world.updateMatrixWorld(true);
    for (const member of standing) member.box = boundingBox(member.robot);

    const widths = standing
      .map((member) => (member.box.isEmpty() ? 0 : member.box.max.x - member.box.min.x))
    ...
    for (const member of standing) {
      ...
      member.offset = cursor - box.min.x;
      cursor = member.offset + box.max.x + gap;
    }
    ...
    for (const member of standing) {
      const box = member.box;
      if (box.isEmpty()) continue;
      // Along the row; standing on the floor; centred in depth, so a machine
      // whose origin is behind it does not sit a metre off the line.
      member.robot.position.x = member.offset - centre;
      member.robot.position.z = -box.min.y;
      member.robot.position.y = (box.min.z + box.max.z) / 2;
      // Where the tag hangs and where the ring is drawn, in the scene's frame.
      member.anchor = new THREE.Vector3(
        (box.min.x + box.max.x) / 2 + member.offset - centre,
        box.max.y - box.min.y,
        0,
      );
      member.radius = Math.max(box.max.x - box.min.x, box.max.z - box.min.z) * 0.62;
      member.floorX = member.anchor.x;
      // What the row is actually standing them at, measured rather than
      // declared — the same reading the registry records per robot. Published
      // for the headless check, which is how "at true scale" is a test rather
      // than a claim.
      member.height = box.max.y - box.min.y;
    }
    this.viewer.world.updateMatrixWorld(true);
    el('cmp-stage').dataset.span = (cursor - gap).toFixed(4);
    this.frame();
    this.paintRing();
  }
  ...
  bindStage() {
    ...
    host.addEventListener('pointerup', (event) => {
      ...
      // Off every robot is how the window is let go: the backdrop is the
      // biggest target on the stage, and in fullscreen it is the only one.
  ...
  paintRing() {
    ...
    if (!member || !member.box || member.box.isEmpty()) return;
    ...
    // the ring's own plane: it needs no rotation of its own, and the floor the
    // row stands on is z = 0 there. The hair above it keeps the two from
    // z-fighting along the whole circle.
    ring.position.set(member.floorX || 0, 0, 0.002);
    ring.renderOrder = 3;
    ...
    host.innerHTML = standing
      .map((member) => {
        ...
        return `<button type="button" class="cmp-tag" data-key="${esc(member.entry.key)}"
          ...
          data-x="${(member.floorX || 0).toFixed(4)}"
